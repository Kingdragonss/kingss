...
client.on("message", message => {
    if(message.content.toLowerCase() == "HYDRA") 
    return message.channel.send(`HYDRA :heart: :heart:`)
});
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('791998307093708814');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('791998307093708817');
  }, ms(süre));
}
})
